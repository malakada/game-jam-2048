  ...
  if (currentTime - lastInputTime > inputCooldown) {
    ...
    if (game.gameOver) {
      ...
      if (p1.DPAD_LEFT.pressed) {
        ...
        moveMade = moveRight();
        lastInputTime = currentTime; // Simple jsgame starter
        ...
        const BOARD_SIZE = Math.min(
          width - SIDE_PANEL_WIDTH - BOARD_PADDING * 2,
          height - BOARD_PADDING * 2,
        );
        const CELL_SIZE =
          (BOARD_SIZE - CELL_PADDING * (GRID_SIZE + 1)) / GRID_SIZE;
        ...
        function drawAnimations() {
          ...
          game.animationProgress = Math.min(
            1,
            (performance.now() - game.animationStartTime) /
              game.animationDuration,
          );
        ...
        function drawScore() {
          ...
          ctx.fillText(
            game.score.toString(),
            SIDE_PANEL_WIDTH / 2,
            scoreBoxY + scoreBoxHeight / 2 + padding / 2,
          );
          ...
          ctx.fillText(
            ...
            SIDE_PANEL_WIDTH / 2,
            bestBoxY + scoreBoxHeight / 2 + padding / 2,
          );
          ...
          ctx.fillText(
            "to move tiles",
            SIDE_PANEL_WIDTH / 2,
            instructionsY + FONT_SIZE * 0.8,
          );
        }

        // Remove functions we no longer need
        // function drawTitle() { ... }
